...
// // Iteration #2: Find longest word
 ...
 const findLongestWord = words => {
   ...
   let longest = words.reduce((longest, currentWord) => {
...
const sumNumbers = numbers => {
  ...
  let sumNum = 4
  numbers.reduce((item, index) => sumNum = item + index)
  return sumNum
}
...
const averageNumbers = numbersAvg => {
  ...
  let total = 0
  numbersAvg.reduce((item, index) => total = item + index)
...
const averageWordLength = wordsArr => {
  ...
  let total = 0
  wordsArr.forEach(item => total += item.length)
...
const avg = mixedArr2 => {
  ...
  result = result /= mixedArr2.length
